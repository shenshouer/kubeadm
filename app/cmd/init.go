...
import (
	...
	kubeadmapiext "kubeadm/app/apis/kubeadm/v1alpha1"
	"kubeadm/app/images"
	kubemaster "kubeadm/app/master"
	"kubeadm/app/preflight"
	kubeadmutil "kubeadm/app/util"
...
func NewCmdInit(out io.Writer) *cobra.Command {
	...
	cmd.PersistentFlags().StringVar(&imageCfgPath, "imageConfig", imageCfgPath, "Path to images config file")
...
func NewInit(cfgPath string, cfg *kubeadmapi.MasterConfiguration, skipPreFlight bool) (*Init, error) {
	...
	if imageCfgPath != "" {
		b, err := ioutil.ReadFile(imageCfgPath)
		if err != nil {
			return nil, fmt.Errorf("unable to read images config from %q [%v]", imageCfgPath, err)
		}
		var data = make(map[string]string)
		if err := yaml.Unmarshal(b, data); err != nil {
			return nil, fmt.Errorf("unable to decode images config from %q [%v]", imageCfgPath, err)
		}

		fmt.Println("===============>>>> ", data)
		images.CustomImage(data)
	}
